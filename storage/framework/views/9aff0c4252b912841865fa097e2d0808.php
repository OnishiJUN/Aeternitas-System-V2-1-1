<?php $__env->startSection('title', 'Time In/Out'); ?>
    ...
    <div class="space-y-6">
            ...
            <?php if($user->employee): ?>
                <div class="mt-4 flex flex-col sm:flex-row items-center justify-center gap-4">
                    <div class="inline-flex items-center bg-blue-50 px-4 py-2 rounded-full">
                        ...
                        <span class="text-blue-800 font-medium"><?php echo e($user->employee->first_name); ?> <?php echo e($user->employee->last_name); ?></span>
                        <?php if($user->employee->department): ?>
                            <span class="text-blue-600 ml-2">• <?php echo e($user->employee->department->name); ?></span>
                        <?php endif; ?>
                    </div>
                    <div class="inline-flex items-center bg-green-50 px-4 py-2 rounded-full">
                        <i class="fas fa-id-badge text-green-600 mr-2"></i>
                        <span class="text-green-800 font-medium">Employee ID: <?php echo e($user->employee->employee_id); ?></span>
                    </div>
                    <?php if($todayAttendance): ?>
                        <div class="inline-flex items-center bg-blue-50 px-4 py-2 rounded-full">
                            <i class="fas fa-clock text-blue-600 mr-2"></i>
                            <span class="text-blue-800 font-medium">Status: <?php echo e(ucfirst($todayAttendance->status)); ?></span>
                        </div>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        ...
        <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-2xl p-8 text-center text-white shadow-lg">
            ...
            <?php if($todayAttendance && $todayAttendance->time_in && !$todayAttendance->time_out): ?>
                <div class="mt-4 pt-4 border-t border-blue-400" id="time-tracking-section">
        ...
        <!-- Today's Summary -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">Today's Summary - <?php echo e(now()->format('M j, Y')); ?></h3>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-3" id="recent-activity">
                <?php if($recentActivity && $recentActivity->count() > 0): ?>
                    <?php $__currentLoopData = $recentActivity; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $record): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-500"><?php echo e($record->date->format('M j, Y')); ?></div>
                                    </div>
                                </div>
                                <div class="text-sm text-gray-500"><?php echo e($record->time_in->format('g:i A')); ?></div>
                            </div>
                        <?php endif; ?>
                        ...
                        <?php if($record->break_start): ?>
                            <div class="flex items-center justify-between p-3 bg-yellow-50 rounded-lg">
                                ...
                                <div class="text-sm text-gray-500"><?php echo e($record->break_start->format('g:i A')); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if($record->break_end): ?>
                            <div class="flex items-center justify-between p-3 bg-orange-50 rounded-lg">
                                ...
                                <div class="text-sm text-gray-500"><?php echo e($record->break_end->format('g:i A')); ?></div>
                            </div>
                        <?php endif; ?>
                        ...
                        <?php if($record->time_out): ?>
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                ...
                                <div class="text-sm text-gray-500"><?php echo e($record->time_out->format('g:i A')); ?></div>
                            </div>
                        <?php endif; ?>
...
async function updateTimeAjax() {
    try {
        const response = await fetch('<?php echo e(route("attendance.current-time")); ?>');
...
function updateWorkingTime() {
    ...
    // First check currentStatus (from API)
    if (currentStatus && currentStatus.time_in && !currentStatus.time_out) {
...
async function loadAttendanceStatus() {
    try {
        const response = await fetch('<?php echo e(route("attendance.status")); ?>');
        const data = await response.json();
...
function updateUI() {
    ...
    if (currentStatus.status === 'not_started' || !currentStatus.time_in) {
        ...
        // Show break section when timed in
        if (breakSection) breakSection.style.display = 'grid';
        
        if (currentStatus.time_out) {
            statusIcon.className = 'inline-flex items-center justify-center w-20 h-20 bg-green-100 rounded-full mb-4';
            statusIcon.innerHTML = '<i class="fas fa-check-circle text-3xl text-green-600"></i>';
            statusTitle.textContent = 'Workday Complete';
            statusMessage.textContent = `Clocked out at ${currentStatus.time_out}`;
            ...
            // Disable break buttons when timed out
            if (breakStartBtn) {
            ...
            statusTitle.textContent = currentStatus.break_start && !currentStatus.break_end ? 'On Break' : 'Currently Working';
            statusMessage.textContent = currentStatus.break_start && !currentStatus.break_end ? 
                `Break started at ${currentStatus.break_start}` : 
            ...
            // Update break buttons - check for active break
...
async function breakStart() {
    ...
    try {
        const response = await fetch('<?php echo e(route("attendance.break-start")); ?>', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
async function breakEnd() {
    ...
    try {
        const response = await fetch('<?php echo e(route("attendance.break-end")); ?>', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
<?php echo $__env->make('layouts.dashboard-base', ['user' => $user, 'activeRoute' => 'attendance.time-in-out'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\internship\Aeternitas-System-V2\resources\views/attendance/time-in-out.blade.php ENDPATH**/ ?>